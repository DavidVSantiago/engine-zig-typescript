...
class Engine {
    ...
    public readonly MAX_CATCH_UP_TICKS = 5; // máximo de tiks de atraso
    ...
    public async changeScene(nextScene: IScene, minTicks: number = 60) {
        ...
        // YIELD DA THREAD (A MÁGICA ACONTECE AQUI)
        // Isso pausa a execução desta função e joga o resto dela para o fim da fila de eventos.
        // Assim, o gameloop consegue finalizar o ciclo atual e o navegador pinta a tela de loading!
    ...
    public gameLoop(currentTime: number) {
        ...
        // se o atraso for maior que o permitido, limita o atraso
        if (elapsed > this.MS_PER_TICK * this.MAX_CATCH_UP_TICKS) {
            elapsed = this.MS_PER_TICK * this.MAX_CATCH_UP_TICKS;
        }

        this.accumulator += elapsed;
        // atrualiza a cena n vezes, de forma a compensar o atraso
        ...
        const alpha = this.accumulator / this.MS_PER_TICK; // calcula o alpha para a interpolacao
        this.currentScene.render(this.ctx, alpha); // renderiza a cena
